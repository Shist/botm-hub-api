...
A lightweight serverless backend for the **BOTM Hub** project, deployed on the Vercel platform.
...
- `api/match.js` — endpoint for fetching multiplayer lobby data. Validates `match_id`, fetches the authorization token from the osu! API, and returns a processed JSON with match results.
...
Легковесный serverless-бэкенд для проекта **BOTM Hub**, задеплоенный на платформе Vercel.
...
- `api/match.js` — эндпоинт для получения данных о мультиплеерном лобби. Валидирует `match_id`, получает токен авторизации osu! API и возвращает обработанный JSON с результатами игр.
